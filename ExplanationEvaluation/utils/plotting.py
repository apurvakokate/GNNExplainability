...
def save_connectivity_graph(graph, edge_weigths, labels, idx, thres_min, dataset, seed, args=None, gt=None, show=False, folder_name = 'numberOfConnectedComponents'):
    """
    """
    # Set thresholds
    ...
    #print("edges thresholded",pos_edges)

    # if (thres_min == -1):
    gt_edges = [(pair[0], pair[1]) for pair in gt[0][idx].T]
    G_expected.add_edges_from(gt_edges)
    connected_expected = nx.number_connected_components(G_expected)

    writer = SummaryWriter('runs/'+str(args.log_folder)+'/'+folder_name+'/connected_components'+ str(idx)+"expected"+str(connected_expected)+"seed"+str(seed))
    max_connected_components = 0
    position_of_max = []
    conns = []
    prev_connected = 0

    for thres_snip in range(1, 21):#edge_weigths.shape[0]+1):

        thres_index = max(int(edge_weigths.shape[0]-thres_snip),0)

        #todo how many edges remaining
        ...
        #print("edges thresholded",np.shape(pos_edges))
        #print("ground_truth", len(gt[0]))

        # if (thres_min == -1):
        G.add_edges_from(pos_edges)
        connected = nx.number_connected_components(G)
        # Todo find length of largest connected component
        # largest_connected = max(nx.strongly_connected_components(G), key=len)
        # print("largest component",largest_connected)
        conns.append(connected)

        # sorted_connected_e = sorted(nx.connected_components(G_expected), key=len, reverse=True)
        # print("largest component expected",G_expected.subgraph(sorted_connected_e[0]).shape)
        # largest_connected_expected = max(nx.strongly_connected_components(G_expected), key=len)
        # print("Expected largest component",largest_connected_expected)
        #print("Ground truth connected components"+str(idx), connected_expected)

        writer.add_scalar('connected_components', connected, np.shape(pos_edges)[0])#thres_index)
        # print("Ground truth size", len(gt_edges))

        # if connected > max_connected_components:
        #     max_connected_components = connected
        #     position_of_max = [thres_index]
        # elif connected == max_connected_components:
        #     position_of_max.append(thres_index)
    ...
    area = np.trapz(conns)
    return area #max_connected_components, position_of_max, area
